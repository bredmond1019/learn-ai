...
### Os 6 Hooks de Ciclo de Vida do Claude Code
...
### Hook Pre-Prompt em Ação

```javascript
// Exemplo de .claude/hooks/pre-prompt.js
...
### Prevenindo Desastres com Pre-Tool-Use

```javascript
// Salvando você de comandos destrutivos
...
### Criando Sistemas Reativos com Post-Tool-Use

```javascript
export default async function postToolUseHook(tool, args, result) {
...
```javascript
// Sistema de notificação baseado em contexto
...
```javascript
export default async function stopHook(reason) {
...
### Preservando Contexto Crítico Durante Compactação

```javascript
export default async function preCompactHook(context) {
...
```javascript
// Validar → Executar → Verificar → Relatar
...
```javascript
// Rastrear tudo, entender padrões, melhorar com o tempo
...
### Template de Configuração de Hooks
