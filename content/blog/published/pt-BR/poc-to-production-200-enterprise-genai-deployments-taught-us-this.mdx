...
<CodeExample
  title="A Stack Pronta para Produção"
  language="text"
  code={`1. Camada de Fundação
...
<CodeExample
  title="A Evolução do Eval"
  language="python"
  code={`# Dia 1: O Teste de Vibe
...
<CodeExample
  title="O Hack de Velocidade Multimodal"
  language="python"
  code={`# Abordagem ingênua: Processar vídeo inteiro
...
<CodeExample
  title="O Antipadrão de Ferramentas"
  language="python"
  code={`# 🤦‍♂️ PARE DE FAZER ISSO
ferramentas = [{
...
<CodeExample
  title="A Realidade dos Custos"
  language="text"
  code={`Plataforma de Processamento de Vídeo:
...
<CodeExample
  ...
  language="python"
  code={`# Contexto básico (todos têm isso)
contexto = {
...
<CodeExample
  title="Geração de UI Just-In-Time"
  language="typescript"
  code={`// Abordagem tradicional
